...
function Base.convert(::Type{Vector{Float64}}, uav_state::UAVState)
    return [getfield(uav_state, f) for f in fieldnames(uav_state)]
end

# convert a vector to a UAVState type
function Base.convert(::Type{UAVState}, uav_state_vector::Vector{Float64})
...
type State
    ...
    function State(uav_states::Vector{UAVState})
        ...
        coordinator = 1 # default is master
        return new(uav_states, num_uavs, signals, responding_states, coordinator)
    end
end
...
function to_polar_non_joint(state::State)
    @assert length(state.uav_states) == 2 "invalid number of uav_states in state: $state"
    ...
    polar_state = [r, theta, pr, vown, vint, own_responding]
    return polar_state
