...
function get_test_uavs()
    ...
    uav1 = UAV(1, actions, joint_actions, policy, coord, false, strategy, act_select, rng, sensor_range_std, sensor_theta_std, sensor_psi_std, sensor_velocity_std, use_sensor_noise)
    uav2 = UAV(2, actions, joint_actions, policy, coord, false, strategy, act_select, rng, sensor_range_std, sensor_theta_std, sensor_psi_std, sensor_velocity_std, use_sensor_noise)
...
function get_test_env()
    ...
    rng = MersenneTwister(1)
    coord_scheme = 1
    e = Environment(max_steps, use_noise, uavs, gauss_sigma, gauss_mu, d, coord_scheme, start_state_generator, 1/4, 1/3, 100., pi / 32, 153., true, rng, rng)
    return e
end

# Environment tests
function test_env_constructor()
    get_test_env()
end
